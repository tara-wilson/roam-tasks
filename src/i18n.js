export const i18n = {
  en: {
    settings: {
      ...
      todayWidgetPlacementDescription:
        "Place the widget at the top or bottom of the DNP, unless a matching block is found",
      todayWidgetIncludeOverdue: "Include overdue tasks in Today widget",
      todayWidgetIncludeOverdueDescription:
    ...
    prompts: {
      ...
      addRepeatOrDate:
        "Add a repeat rule or at least one start/defer/due date.",
      form: {
        taskLabel: "Task *",
    ...
    dashboard: {
      ...
      bulk: {
        ...
        snoozedCount: (n, days) =>
          `Snoozed ${n} task${n === 1 ? "" : "s"} by ${days} day${days === 1 ? "" : "s"}`,
        updatedCount: (field, n) =>
          `Updated ${field} on ${n} task${n === 1 ? "" : "s"}`,
        fieldLabels: {
          project: "Project",
          waitingFor: "Waiting for",
          context: "Context",
  ...
  zh: {
    settings: {
      ...
      picklistExcludesAdvancedDescription:
        "显示用于从项目/情境/等待列表中排除特定页面的设置（始终排除 roam/* 页面）。",
      ...
      pillThresholdDescription:
        "当复选框数量超过此值时，内联徽标将跳过初始渲染（默认 100）。更高的值会渲染但可能使页面变慢。",
    ...
    prompts: {
      ...
      cannotParseDue:
        "无法解析截止日期。请确保使用 Roam 的标准日期格式（例如 [[2025年11月8日]]）。",
    ...
    toasts: {
      ...
      couldNotParseRecurrence:
        "无法解析任务重复模式。请检查任务并查看 README 中的支持模式。",
      snoozeFailed: "无法延后任务。",
  ...
  zhHant: {
    settings: {
      ...
      todayBadgeLabelDescription:
        "Today 徽章/連結的文字（比對時會忽略 Markdown）",
      ...
      picklistExcludesAdvancedDescription:
        "顯示用於從專案/情境/等待清單中排除特定頁面的設定（永遠排除 roam/* 頁面）。",
      ...
      pillThresholdDescription:
        "當勾選框數量超過此值時，內嵌標籤會跳過初始渲染（預設 100）。較高數值會渲染但可能讓頁面變慢。",
    ...
    prompts: {
      ...
      cannotParseDue:
        "無法解析到期日。請使用 Roam 的標準日期格式（例如 [[2025年11月8日]]）。",
    ...
    toasts: {
      ...
      couldNotParseRecurrence:
        "無法解析任務重複模式。請檢查任務並查看 README 中的支援模式。",
      snoozeFailed: "無法延後任務。",
